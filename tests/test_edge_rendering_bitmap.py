...
# Tests that the function correctly converts a slice of alternating zeros and ones to a Strip object
def test_alternating_zeros_and_ones():
    slice = bitarray('0101010101')
    expected_strip = Strip(segments=[Spacer(width=1), Segment("*"), Spacer(width=1), Segment("*"), Spacer(width=1), Segment("*"), Spacer(width=1), Segment("*"), Spacer(width=1), Segment("*")])
    assert _slice_to_strip(slice) == expected_strip
